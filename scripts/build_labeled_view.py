"""Corpus view for the BASELINE artifact: the human-labeled docs
(camgeodesic/metagaming-labeling annotations) x the OG blocklist features (222).
...
docs_meta = {d["id"]: d for d in map(json.loads, open(f"{ROOT}/data/labeled_eval_docs.jsonl"))}

AXES = [
    {"key": "cot_probe_hq", "name": "Apollo CoT (annotated)", "short": "cot", "cssv": "--ax-dm",
     "desc": "Schoen/SA reasoning traces with human metagaming spans"},
    {"key": "chat_kimi", "name": "Kimi chat rollouts (annotated)", "short": "chat", "cssv": "--ax-mc",
     "desc": "chat-register rollouts with human metagaming spans"},
    {"key": "other_positive", "name": "Prose docs (annotated)", "short": "prose", "cssv": "--ax-dg",
     "desc": "metagaming-probe + Westover docs with human spans"},
    {"key": "controls", "name": "Controls (reviewed)", "short": "control", "cssv": "--ax-co",
     "desc": "control docs reviewed by the annotator (mostly clean)"},
]
def axis_of_group(g):
    if g in ("cot_probe_hq", "chat_kimi"): return g
    if g in ("metagaming_probe_docs", "westover_infocus"): return "other_positive"
    return "controls"
...
status = np.array([docs_meta[i]["status"] for i in ids])
posm, ctlm = status == "done", status == "clean"
def auc_l(l):
...
for i, did in enumerate(ids):
    ...
    ax = axis_of_group(dm["group"])
    docs_out.append({
        "i": i, "id": did, "line": i + 1, "source": dm["group"], "model": None,
        "env": dm["status"], "category": f"{dm['group']} · {dm['status']} · {len(dm['char_spans'])} spans",
        "axis": ax, "label": "positive" if dm["char_spans"] else "control",
        "n_tokens": int(ntok[i]), "n_chars": len(dm["text"]), "has_context": False,
        "fires": keep[:45], "n_latents_firing": int((max_dl[i] > 0).sum()),
        "prov_items": [f"Human-annotated doc from camgeodesic/metagaming-labeling (doc_set {dm['group']}, status {dm['status']}, {len(dm['char_spans'])} spans)",
                        f"~/sae-exploration/data/labeled_eval_docs.jsonl · line {i + 1}"],
        "prov_cmd": f"sed -n '{i + 1}p' ~/sae-exploration/data/labeled_eval_docs.jsonl | jq -r '.text'",
    })
...
tv = {"latents": OG, "docs": tv_docs,
      ...
      "stats_note": ("Baseline: the OG blocklist (222 regex-flagged features), thresholds "
                     "mean+4SD over this labeled corpus. Underlined text = human-annotated "
                     "metagaming span (ground truth)."),
      "rule_note": "Rathi & Radford 2601.21571 §5.1"}
out = {"sae": "labeled-baseline-103", "model": "unsloth/gemma-3-27b-it", "layer": 40, "n_layers": 62,
...
json.dump(out, open(f"{ROOT}/out/corpus_view_labeled.json", "w"), separators=(",", ":"))
print(f"labeled view: {len(docs_out)} docs, {len(latents)} latents, "
