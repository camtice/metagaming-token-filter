...
data = {"saes": [encode_tv(json.load(open(f"{ROOT}/out/corpus_view_test_v4.json")))]}
blob = json.dumps(data, separators=(",", ":")).replace("</", "<\\/")
tpl = open(f"{ROOT}/artifact/test_firing_map.template.html").read()
assert "/*__DATA__*/" in tpl
html = tpl.replace("/*__DATA__*/", blob, 1)
open(f"{ROOT}/out/test_firing_map.html", "w").write(html)
print(f"wrote out/test_firing_map.html ({len(html)/1e6:.2f} MB)")
